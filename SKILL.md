...
> **注意：浏览器工具依赖**
> 本 skill 中的 `BROWSER_TOOL` 命令表示需要一个无头浏览器工具来执行操作。
> 请根据你使用的 Coding Agent 替换为对应的浏览器工具命令（如 Playwright、Puppeteer、
> 或其他可用的浏览器自动化工具）。常用操作映射：
> - `BROWSER_TOOL goto <url>` — 导航到 URL
> - `BROWSER_TOOL screenshot <path>` — 截图
> - `BROWSER_TOOL snapshot -i` — 获取页面交互元素
> - `BROWSER_TOOL click <selector>` — 点击元素
> - `BROWSER_TOOL fill <selector> <value>` — 填写表单
> - `BROWSER_TOOL text` — 获取页面文本
> - `BROWSER_TOOL console` — 查看控制台日志
...
ls ~/.skill-data/projects/$SLUG/*brainstorm* 2>/dev/null | head -5
...
If `NEEDS_SETUP`:
1. Tell the user: "浏览器工具需要初始化。请确保你的环境中有可用的浏览器自动化工具（如 Playwright）。"
...
```bash
BROWSER_TOOL goto "https://example-site.com"
BROWSER_TOOL screenshot "/tmp/design-research-site-name.png"
